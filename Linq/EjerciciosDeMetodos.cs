...
List<string> dobleFiltro = catalogo.Where(p => p.EsMultijugador ==true)
.Select(p => p.Titulo).ToList();//no supe como hacerlo con el tipo objeto, no sabia donde va el tolist
